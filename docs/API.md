...
| `GET /v1/repos/:repo/kicad/board?ref=&path=&layer=&zones=` | KiCad board: index without `layer`, one layer with it. The index may start an on-demand mesh build and report `models.building` (ADR-040) |
