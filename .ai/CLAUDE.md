...
See `AGENTS.md` for the canonical, tool-agnostic version of these rules.

## No AI / agent attribution

Do not add any AI- or coding-agent attribution, branding, or metadata to anything
in this repository. This applies to every artifact an agent produces or edits,
including but not limited to:

- **Commit messages** — no `Co-Authored-By:` for an AI assistant/model, no
  "Generated with …" lines, no session links, no agent/model trailers.
- **Pull requests** — no agent attribution in titles or descriptions ("Generated
  with …", tool/session links, badges).
- **Code** — no comments crediting or referencing a coding agent/model
  ("written by", "generated by", AI signatures).
