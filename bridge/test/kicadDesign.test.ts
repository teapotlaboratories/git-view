...
function solve(files: Record<string, string>, top = "/d/top.kicad_sch") {
  const design = loadDesign(top, (f) => {
    const hit = files[f];
...
test("busMembers expands vectors and groups, and leaves plain names alone", () => {
...
test("a sheet pin binds to the child's hierarchical label of the same name", () => {
  const files = {
  ...
  assert.deepEqual(solve(files), [{ name: "SIG", pins: ["R1.1", "R2.1"] }]);
});

test("same-named sheet pins on different children are NOT shorted together", () => {
  ...
  assert.deepEqual(solve(files), [
    { name: "BLUE", pins: ["R1.1", "RA.1"] },
    { name: "BLUE", pins: ["R2.1", "RB.1"] },
  ]);
});

test("a reused sheet takes its references from the instance path, not the Reference property", () => {
...
test("a global label crosses sheets; a local label of the same name does not", () => {
  const files = {
    "/d/top.kicad_sch": sheetFile("root", [TP], [
      inst("t:TP", "R1", 0, 10),
      wire(0, 10, 20, 10),
...
test("a power symbol's net name comes from its pin, not its Value", () => {
...
test("a bus sheet pin carries its members into a child that names them plainly", () => {
  ...
  assert.deepEqual(solve(files), [{ name: "AN0", pins: ["R1.1", "R2.1"] }]);
});

test("two differently-named buses that meet pair their members up by index", () => {
...
test("bus geometry never leaks into signal connectivity", () => {
...
test("an unreadable sub-sheet is reported, not silently dropped", () => {
...
test("a healthy design reports no problems", () => {
  const files = {
