...
test("a directory is not a converter", () => {
  // A realistic typo in `kicad.converter`. `existsSync` accepted it, so the bridge reported `running`
  // forever — node exits non-zero on a directory every time — instead of degrading to "cannot convert".
  assert.equal(findConverter(mkdtempSync(join(tmpdir(), "gv-dir-"))), undefined);
});
